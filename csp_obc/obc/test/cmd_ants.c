...
int cmd_ants_status(struct command_context *ctx) {
	...
	ret = isis_ants_status(cmd_antx_current_addr, &status);
	...
	ret = isis_ants_temp(cmd_antx_current_addr, &temp_data);
...
int cmd_ants_countdown_val(struct command_context *ctx) {
	char * args = command_args(ctx);
	unsigned int value;
	if (args == NULL || sscanf(args, "%d", &value) != 1)
		return CMD_ERROR_SYNTAX;

	if (!isis_ants_countdown_val(value))
...
command_t __sub_command ant_subcommands[] = {
	{
		...
		.name = "countdownset",
		.help = "Write new countdown value",
		.usage = "<value>",
		.handler = cmd_ants_countdown_val,
	},
};
