...
int cmd_rtc_get(struct command_context *ctx __attribute__((unused))) {
	...
	printf("RTC Time is: %s", ctime((time_t *) &time));

	time = clock_get_time_nopara();

	printf("SYS Time is: %s", ctime((time_t *) &time));

	return CMD_ERROR_NONE;
}

int cmd_sys_get(struct command_context *ctx __attribute__((unused))) {
...
int cmd_rtc_read(struct command_context *ctx __attribute__((unused))) {
