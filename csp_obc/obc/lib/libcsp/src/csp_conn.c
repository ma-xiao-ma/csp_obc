...
/* 连接池访问锁 */
static csp_bin_sem_handle_t conn_lock; //二值信号量

/* 源端口 */
static uint8_t sport;

/* 源端口锁 */
static csp_bin_sem_handle_t sport_lock; //二值信号量
...
csp_conn_t * csp_connect(uint8_t prio, uint8_t dest, uint8_t dport, uint32_t timeout, uint32_t opts) {
	...
	while (++sport != start) {
		...
		/* 寻找与incoming目的端口匹配的已打开的连接 */
		conn = csp_conn_find(incoming_id.ext, CSP_ID_DPORT_MASK);

		/* 如果没找到对应的连接，说明此sport还未使用，可以使用此端口 */
		if (conn == NULL)
			break;
	}
